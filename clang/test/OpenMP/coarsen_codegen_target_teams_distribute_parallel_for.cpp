...
//
// IR3:  call void @__kmpc_for_static_init_
// IR3: call{{.*}}@body(
...
#pragma omp parallel
        {
#pragma omp for ompx_coarsen_for(3) schedule(static, 2)
          for (int j = startj; j < endj; j+=stepj) {
            body(start, end, step, i,
                 startj, endj, stepj, j);
          }
...
// RUN: %clang_cc1 -DCK6 -fopenmp -x c++ -std=c++11 -triple amdgcn-amd-amdhsa -fopenmp-targets=amdgcn-amd-amdhsa -emit-llvm %s -fopenmp-is-target-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s --check-prefix=IR3
