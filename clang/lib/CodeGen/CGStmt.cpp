...
void CodeGenFunction::EmitForStmt(const ForStmt &S,
  ...
  // Emit transformer for locs
  for (const Attr *A : ForAttrs) {
    auto TLA = dyn_cast<TransformLabelAttr>(A);
    if (!TLA)
      continue;
    ...
    ForLocs.push_back(LocInfo);
    llvm::ArrayType *ATy =
        llvm::ArrayType::get(LocInfo->getType(), ForLocs.size());
    GV = new llvm::GlobalVariable(CGM.getModule(), ATy, false,
                                  llvm::GlobalValue::AppendingLinkage,
                                  llvm::ConstantArray::get(ATy, ForLocs), Name);
    GV->setSection("llvm.metadata");
    // TODO do we need to appendToUsed? will erasing it break because it has
    // uses? or does the section prevent it from being erased?
  }
