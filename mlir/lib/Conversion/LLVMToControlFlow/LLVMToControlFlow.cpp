...
struct ConvertLLVMToControlFlow
    : public impl::ConvertLLVMToControlFlowPassBase<ConvertLLVMToControlFlow> {
  using Base::Base;
...
std::unique_ptr<Pass> mlir::createConvertLLVMToControlFlowPass() {
  return std::make_unique<ConvertLLVMToControlFlow>();
}
