...
static void getIndVars(Operation *op, SmallPtrSet<Value, 3> &indVars) {
  if (auto pop = dyn_cast<scf::ParallelOp>(op))
    ...
    for (auto var :
         cast<affine::AffineParallelOp>(op).getBody()->getArguments())
      indVars.insert(var);
}
...
bool mayWriteTo(Operation *op, Value val, bool ignoreBarrier) {
  ...
  if (ignoreBarrier && isa<affine::AffineBarrierOp>(op))
    return false;
...
bool collectEffects(Operation *op,
  ...
  // this barrier again).
  if (ignoreBarriers && isa<affine::AffineBarrierOp>(op))
    return true;
...
bool getEffectsBefore(Operation *op,
  ...
  if (op != &op->getBlock()->front())
    for (Operation *it = op->getPrevNode(); it != nullptr;
         it = it->getPrevNode()) {
      if (isa<affine::AffineBarrierOp>(it)) {
        if (stopAtBarrier)
          return true;
        else
          continue;
...
static bool isRecomputableAfterDistribute(Operation *op,
  ...
  if (isa<affine::AffineBarrierOp>(op))
    return false;
  ...
  while (true) {
    ...
    if (isa<affine::AffineBarrierOp>(prev))
      break;
    begin = prev;
  }
  ...
  while (true) {
    ...
    if (end == nullptr)
      break;
    if (isa<affine::AffineBarrierOp>(end))
      break;
  }
...
static void minCutCache(affine::AffineBarrierOp barrier,
...
static void findValuesUsedBelow(affine::AffineBarrierOp op,
...
static affine::AffineBarrierOp getFirstBarrier(Block *block) {
  auto it = llvm::find_if(block->getOperations(), [](Operation &nested) {
    return isa<affine::AffineBarrierOp>(nested);
  });
  if (it == block->end()) {
    return nullptr;
  }
  return cast<affine::AffineBarrierOp>(&*it);
}

template <typename T, bool UseMinCut>
static LogicalResult distributeAroundFirstBarrier(T op, T &preLoop, T &postLoop,
                                                  PatternRewriter &rewriter) {
  affine::AffineBarrierOp barrier = getFirstBarrier(op.getBody());
  if (!barrier)
    return failure();
...
bool isBarrierContainingAll(Operation *op, SmallVector<BlockArgument> &args) {
  auto bar = dyn_cast<affine::AffineBarrierOp>(op);
  if (!bar)
    return false;
...
static LogicalResult wrapWithBarriers(T op, PatternRewriter &rewriter,
  ...
  if (!hasPrevBarrierLike)
    before = rewriter.create<affine::AffineBarrierOp>(op->getLoc(), vargs);

  if (!hasNextBarrierLike) {
    OpBuilder::InsertionGuard guard(rewriter);
    rewriter.setInsertionPointAfter(op);
    after = rewriter.create<affine::AffineBarrierOp>(op->getLoc(), vargs);
  }
...
static LogicalResult wrapWithBarriers(T op, PatternRewriter &rewriter,
                                      SmallVector<BlockArgument> &args,
                                      bool recomputable) {
  affine::AffineBarrierOp before, after;
...
distributeAfterWrap(Operation *pop, affine::AffineBarrierOp barrier,
  ...
  if (auto cast = dyn_cast<T>(pop))
    return distributeAroundBarrier<T, UseMinCut>(cast, barrier, preLoop,
                                                 postLoop, rewriter, postPop);
  else
    return failure();
}
...
static LogicalResult wrapAndDistribute(T op, bool singleExecution,
  ...
  // immediately to prevent BarrierElim from eliminating them
...
struct WrapWhileWithBarrier : public OpRewritePattern<scf::WhileOp> {
  ...
  LogicalResult matchAndRewrite(scf::WhileOp op,
    ...
    if (op.getNumOperands() != 0 || op.getNumResults() != 0) {
      LLVM_DEBUG(
          DBGS() << "[wrap-while] ignoring non-polygeist-mem2reg'd loop ops: "
...
template <typename ParallelOpType>
static void moveBodies(PatternRewriter &rewriter, ParallelOpType op,
                       affine::AffineForOp forIf,
                       affine::AffineForOp newForIf) {
  moveBodiesFor(rewriter, op, forIf, newForIf);
}
...
static affine::AffineBarrierOp getBarrierInBlock(Block *block) {
  for (Operation &op : *block) {
    if (auto barrier = dyn_cast<affine::AffineBarrierOp>(&op))
      return barrier;
  }
  return nullptr;
}
...
struct DistributeIfAroundBarrier : public OpRewritePattern<IfOpType> {
  ...
  LogicalResult matchAndRewrite(IfOpType ifOp,
    ...
    affine::AffineBarrierOp thenBarrier = nullptr;
    affine::AffineBarrierOp elseBarrier = nullptr;
    if (thenBlock)
    ...
    if (!thenBarrier) {
      ...
      thenBarrier = rewriter.create<affine::AffineBarrierOp>(
          ifOp->getLoc(), elseBarrier->getOperands());
    }
    if (elseBlock) {
      if (!elseBarrier) {
        rewriter.setInsertionPoint(getElseBlock(ifOp)->getTerminator());
        elseBarrier = rewriter.create<affine::AffineBarrierOp>(
...
struct Reg2MemFor : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    while (IP != &IP->getBlock()->front()) {
      if (isa<affine::AffineBarrierOp>(IP->getPrevNode())) {
        IP = IP->getPrevNode();
      }
      break;
    }
...
void addPatterns(RewritePatternSet &patterns, StringRef method,
  ...
  if (method.contains("ifhoist")) {
    ...
    if (method.contains("ifsplit")) {
      patterns.insert<
          DistributeIfAroundBarrier<scf::IfOp, affine::AffineParallelOp,
                                    UseMinCut>,
          DistributeIfAroundBarrier<affine::AffineIfOp,
