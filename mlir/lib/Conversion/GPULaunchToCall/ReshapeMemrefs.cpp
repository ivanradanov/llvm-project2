...
LogicalResult reshapeAtAddr(memref::AtAddrOp &atAddr) {
  ...
  return reshapeMemref(memref, [&](RewriterBase &rewriter, unsigned resultId,
    ...
    atAddr = rewriter.replaceOpWithNewOp<memref::AtAddrOp>(
        atAddr, atAddr.getAddr(), newMt);
  });
}
...
struct ReshapeMemrefsPass
  ...
  void runOnOperation() override {
    ...
    {
      SmallVector<memref::AtAddrOp> toHandle;
      op->walk([&](memref::AtAddrOp atAddr) { toHandle.push_back(atAddr); });
