#ifndef MLIR_LIB_CONVERSION_GPULAUNCHTOCALL_UTILS_H_
#define MLIR_LIB_CONVERSION_GPULAUNCHTOCALL_UTILS_H_
...
static inline mlir::Value bitcastToVec(mlir::RewriterBase &rewriter,
  ...
  if (isa<LLVM::LLVMPointerType>(ty)) {
    ...
    v = rewriter.create<LLVM::PtrToIntOp>(v.getLoc(), intTy, v);
    return rewriter.create<LLVM::BitcastOp>(v.getLoc(), vty, v);
  } else {
    return rewriter.create<LLVM::BitcastOp>(v.getLoc(), vty, v);
  }
}
...
createAffineVectorLoad(mlir::RewriterBase &rewriter, mlir::DataLayout &dl,
                    mlir::Location loc, mlir::Type ty, Args &&...args) {
  using namespace mlir;
  auto vty = VectorType::get({(int64_t)dl.getTypeSize(ty)},
                             rewriter.getI8Type());
  auto load =
      rewriter.create<affine::AffineVectorLoadOp>(loc, vty, std::forward<Args>(args)...);
  return bitcastFromVec(rewriter, dl, ty, load);
}
...
#endif // MLIR_LIB_CONVERSION_GPULAUNCHTOCALL_UTILS_H_
