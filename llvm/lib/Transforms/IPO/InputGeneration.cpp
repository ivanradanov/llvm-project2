...
void InputGenInstrumenter::renameGlobals(Module &M, TargetLibraryInfo &TLI) {
...
bool InputGenInstrumenter::instrumentModule(Module &M) {
...
bool InputGenInstrumenter::instrumentEntryPoint(Module &M, Function &EntryPoint,
                                                bool UniqName) {
  ...
  case IG_Record:
    ...
    break;
  case IG_Generate:
    createGenerationEntryPoint(EntryPoint, UniqName);
    break;
  case IG_Replay:
    createRunEntryPoint(EntryPoint, UniqName);
    break;
  }
...
bool InputGenInstrumenter::instrumentModuleForFunction(Module &M,
                                                       Function &EntryPoint) {
...
bool InputGenInstrumenter::instrumentFunctionPtrs(Module &M) {
  if (ClInstrumentFunctionPtrs)
    instrumentFunctionPtrSources(M);
  provideFunctionPtrGlobals(M);
...
void InputGenInstrumenter::setABIInfo(TY &CorF,
                                      const SmallVector<ABIAttrs> &ABIInfo,
                                      IRBuilder<> &IRB) {
  auto DL = M.getDataLayout();
  auto &Ctx = CorF.getContext();
  int Idx = 0;
  for (auto &ArgInfo : ABIInfo) {
    Type *Ty = nullptr;
    if (ArgInfo.StructRet) {
      CorF.addParamAttr(
          Idx, Attribute::getWithStructRetType(Ctx, ArgInfo.StructRet));
      Ty = ArgInfo.StructRet;
    }
    if (ArgInfo.InAlloca) {
      CorF.addParamAttr(Idx,
                        Attribute::getWithInAllocaType(Ctx, ArgInfo.InAlloca));
      Ty = ArgInfo.InAlloca;
    }
    if (ArgInfo.ByVal) {
      CorF.addParamAttr(Idx, Attribute::getWithByValType(Ctx, ArgInfo.ByVal));
      Ty = ArgInfo.ByVal;
    }
    if (ArgInfo.SwiftSelf)
      CorF.addParamAttr(Idx, Attribute::SwiftSelf);
    if (Ty && Mode == IGInstrumentationModeTy::IG_Generate) {
      Value *Addr = nullptr;
      if constexpr (std::is_same<TY, Function>::value) {
        Addr = CorF.getArg(Idx);
      } else if constexpr (std::is_same<TY, CallInst>::value) {
        Addr = CorF.getArgOperand(Idx);
      } else {
